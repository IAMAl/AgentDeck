...
interface PendingPrompt {
  ...
  checked: Set<number>;
  /** Identity of the prompt the ticks belong to (question + option set). The
   *  bridge re-emits the same prompt on every cursor move, so ticks must carry
   *  across those; but two DIFFERENT prompts that happen to share question text
   *  (e.g. two sub-questions of one AskUserQuestion) must NOT inherit each
   *  other's ticks — the option set distinguishes them. */
  signature: string;
}

/** Prompt identity for tick carry — question text plus the option set, so a
 *  new question with the same text but different options reads as different.
 *  JSON.stringify gives a collision-free, plain-text key: nested arrays can't
 *  be forged by any question or label content the way a delimiter string can. */
function promptSignature(question: string | undefined, options: PromptOption[]): string {
  return JSON.stringify([question ?? '', options.map((o) => [o.index, o.label])]);
}
...
export function updateOptionSelectDial(
  ...
  // Carry the tick set across state updates for the SAME prompt — the bridge
  // re-emits on every cursor move, and rebuilding the set each time would erase
  // everything the user ticked between the first press and the confirm tap. The
  // identity is the full prompt signature (question + option set), NOT the
