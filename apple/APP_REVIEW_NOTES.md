...
AgentDeck Dashboard is a real-time monitoring and evaluation app for AI coding agents (Claude Code, opt-in Codex lifecycle hooks, opt-in OpenCode local-server monitoring, and OpenClaw Gateway sessions). The submitted macOS app's built-in Swift daemon owns the dashboard server, hook ingestion, session state, local-network pairing, APME Layer 2 evaluation, and native hardware modules. It shows live status, tool activity, and quality scores on Mac and on the paired iOS companion. When an agent genuinely waits on a question, AgentDeck shows the choices the agent itself supplied. The user may answer from the app, in which case the app replies to that agent's own local hook request with the option the user chose; if nobody answers, the request is released untouched and the agent's normal prompt appears in the user's terminal. AgentDeck never invents a choice and never proceeds on the user's behalf.
