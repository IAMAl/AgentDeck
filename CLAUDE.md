...
- **plugin/** — Stream Deck SDK v2 plugin. Five actions: `session-slot` (keypad) + four SD+ encoders — E1 Volume (`utility-dial`), E2 Claude Usage (`option-dial`), E3 Codex Usage (`iterm-dial`), E4 Launcher (`launcher`). UUIDs are immutable post-distribution so several no longer match their display name; the mapping is in [docs/streamdeck-layout.md](docs/streamdeck-layout.md). macOS-only, `SDKVersion: 3` — Maker Console rejects 2 ("Minimum Manifest SDK version must be 3 or later") and DRM follows from the SDK version, so neither is optional; verify the DRM-processed build's encoders through the review loop before publishing (see streamdeck-layout)
