...
function buildHookCommand(eventName: string): string {
  return [
    ...
    `fi`,
    `PORT="\${PORT:-9120}"`,
    // Bounded timeouts: an unbounded POST to a restarting daemon outlives
    // Claude's ~1.5s SessionEnd abort budget and surfaces as
    // `SessionEnd hook [...] failed: Hook cancelled`. See hooks/src/install.ts.
